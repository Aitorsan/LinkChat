...
public class Protocol {
    ...
    static public void sendServerErrorMessage(String message,  OutputStream out) throws IOException {
        
        if (message.isEmpty()){
            throw new IOException("Can't send empty error message");
        }
         // write length prefix
        ...
        // 4 bytes length | 1 byte code | error message
        ByteBuffer buffer = ByteBuffer.allocate(5 + message.getBytes().length);
        buffer.putInt(message.getBytes().length);
        buffer.put(fromTypeEnumToByte(FrameType.ERROR));
        buffer.put(message.getBytes());
        out.write(buffer.array());
        out.flush();
    }

    static public void sendServerInvalidUserNameMessage(String message,  OutputStream out) throws IOException {
        ...
        // 4 bytes lenght | 1 byte code | message
        ByteBuffer buffer = ByteBuffer.allocate(5 + message.getBytes().length);
        buffer.putInt(message.getBytes().length);
        buffer.put(fromTypeEnumToByte(FrameType.INVALID_USER_NAME));
        buffer.put(message.getBytes());
        out.write(buffer.array());
        out.flush();
    }
    ...
    static public void sendUserState(String usersConnectedList, FrameType type,  OutputStream out) throws IOException {
        
        if (type != FrameType.USER_CONNECTED && type != FrameType.USER_DISCONNECTED){
            throw new IOException("[ERROR]Wrong Message type to send user state:"+ fromTypeEnumToByte(type) );
        }
        if (usersConnectedList.isEmpty()){
            throw new IOException("Can't send empty verification message");
        }
         // write length prefix
         if (usersConnectedList.length() > MAX_MESSAGE_LENGTH) {
            throw new IOException("message too long");
        }

        // 4 bytes lenght | 1 byte code | users connected list
        ByteBuffer buffer = ByteBuffer.allocate(5 + usersConnectedList.getBytes().length);
        buffer.putInt(usersConnectedList.getBytes().length);
        buffer.put(fromTypeEnumToByte(type));
        buffer.put(usersConnectedList.getBytes());
        out.write(buffer.array());
        out.flush();
    }


    static synchronized public void  sendMessage(Message msg , OutputStream out) throws IOException {
        ...
        // 4 bytes lenght | 1 byte code | 4 byte destiny size name| 4 bytes from size name|  Destinatary + from +  Message
        ByteBuffer buffer = ByteBuffer.allocate(13 + msg.from.length() + msg.destiny.length()  + msg.length);
    ...
    static public Message readMessage(DataInputStream in) throws IOException {
        ...
        if (msg.type == FrameType.MESSAGE) {
            ...
            if (destinyNameSize > MAX_MESSAGE_LENGTH){
                throw new IOException("Size of the destination name is too long: " + destinyNameSize);
            }
